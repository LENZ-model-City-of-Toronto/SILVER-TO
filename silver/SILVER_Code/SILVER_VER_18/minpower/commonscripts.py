...
def ts_from_csv(filename, index_col=0, squeeze=True, timezone=None, is_df=True, **kwargs):
    ...
    df = pd.read_csv(filename, index_col=index_col, squeeze=squeeze, skip_blank_lines=True, **kwargs)
...
def transpose(listoflists):
    return (list(map(list, list(zip(*listoflists)))))
...
def remove_leading_zeros_time_formatting(startdate, enddate):

    try:
        startdate = '{d.month}/{d.day}/{d.year} {d.hour}:{d.minute:02}'.format(d=startdate)
        enddate = '{d.month}/{d.day}/{d.year} {d.hour}:{d.minute:02}'.format(d=enddate)

    except ValueError:
        print(f"incorrect user defined startdate: {startdate} or enddate: {enddate} string format. It should be YYYY-MM-DD")
