...
class TimeIndex(object):
    ...
    def __init__(self, index, str_start=0):
        ...
        # print "self.strings looks like: ", self.strings
    ...
    def get_interval(self):
        ...
        if freq is not None:
            ...
            if self.interval.freqstr == 'H':
                ...
                self.intervalhrs = old_div(self.interval.nanos, 1.0e9 / 3600.0)
        else:
    ...
    def subdivide(self, division_hrs=24, overlap_hrs=0):
        int_division = int(old_div(division_hrs, self.intervalhrs))
        int_overlap = int(old_div(overlap_hrs, self.intervalhrs))
        subsets = []
        for stg in range(int(old_div(len(self), int_division))):
