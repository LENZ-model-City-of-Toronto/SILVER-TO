...
class Bid(OptimizationObject):
    ...
    def build_model(self):
        if self.bid_points is None:
            ...
            in_pts = {t: self.discrete_input_points for t in self.times.set}
    ...
    def output(self, time=None, scenario=None, evaluate=False):
        ...
        if self.is_linear:
            ...
            # self.polynomial[1] is 60 for example in the case of g0, as read in from generators.py
            # power is power[t00] for example
            # as such the self.polynomial[1] is the one that you ned to change in the case of storage
            # technologies to make it the cost of electricity.
    ...
    def output_true(self, input_var, force_linear=False):
        ...
        if self.is_linear or (not self.is_pwl and not force_linear):
            return polynomial_value(self.polynomial, input_var)
        ...
        for A, B in pairwise(self.bid_points.values.tolist()):
            if A[0] <= input_var <= B[0]:
                return get_line_value(A, B, input_var) + self.constant_term

    def output_incremental(self, input_var):
        if not self.is_pwl:
            return polynomial_incremental_value(self.polynomial, (input_var))
    ...
    def output_incremental_range(self):
        if self.is_pwl:
            ...
            output_range = [0]
            output_range.extend(get_line_slope(A, B) for A, B in pairwise(self.bid_points))
        else:
...
def parse_polynomial(s):
    ...
    def parse_p(s, powerPattern):
        ...
        return int(multipliers) if multipliers else 1
...
def get_line_slope(A, B):
    xA, yA = A
    xB, yB = B
    return old_div((yB - yA) * 1.0, (xB - xA))
